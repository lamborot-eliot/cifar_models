...
class Block(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, expansion, stride):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=1, groups=planes, bias=False) # don't do decomposition on group layers.
...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, alpha=1.0, num_classes=10):
        ...
        self.c1stride = 1 if self.num_classes==10 else 2 
        self.pooling_kernel = 4 if self.num_classes==10 else 7
        last_channel = int(1280 * alpha) if alpha > 1.0 else 1280
        self.cfg = [(1,  16, 1, 1),
               (6,  24, 2, self.c1stride),  # NOTE: change stride 2 -> 1 for CIFAR10
               (6,  32, 3, 2),
               (6,  64, 4, 2),
               (6,  96, 3, 1),
               (6, 160, 3, 2),
               (6, 320, 1, 1)]
        self.conv1 = nn.Conv2d(3, int(32*alpha), kernel_size=3, stride=self.c1stride, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        out = F.avg_pool2d(out, self.pooling_kernel)
