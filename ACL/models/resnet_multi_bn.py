...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        print("I removed this for compatatibily reasons with old torchvsion versions")
...
def resnet18_activation(pretrained=False, progress=True, activationFunction=torch.nn.ReLU, **kwargs):
    ...
    model =  _resnet('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs)
    model.relu = activationFunction(inplace=True)
    model.layer1[0].relu = activationFunction(inplace=True)
    model.layer1[1].relu = activationFunction(inplace=True)
    model.layer2[1].relu = activationFunction(inplace=True)
    model.layer2[1].relu = activationFunction(inplace=True)
    model.layer3[1].relu = activationFunction(inplace=True)
    model.layer3[1].relu = activationFunction(inplace=True)
    model.layer4[1].relu = activationFunction(inplace=True)
    model.layer4[1].relu = activationFunction(inplace=True)
    return model
